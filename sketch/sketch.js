	...
	if (list.length > 0) {
		...
		for (var a = 0; a < list.length; a++) {
			...
			if (a != no) {
				...
				var distance = dis (list[no].x, list[no].y, list[a].x, list[a].y);

				sLength[sLength.length] = {val: a, position: distance};
			}
		}
		...
		var endV = (sLength.length < 8 ? sLength.length : 8);

		var arr = [];

		for (var v = 0; v < endV; v++) arr[v] = sLength[v].val;
...
function loopR () {
	...
	for (var v = 0; v < list.length; v++) holdR[v] = spaceReu (v);

	catchV = cloneArray (holdR);
}
...
function extra (save, removed, cnt, aV, distance, dF, rL, cap) {
	...
	var side = catchV[removed[cnt].pos];
	
	if (aV < side.length) {

		if (save[side[aV]] != null) {

			var combine = distance + 
				dis (removed[cnt].val.x, removed[cnt].val.y, save[side[aV]].x, save[side[aV]].y);
			...
			cR[addOn] = {val: list[side[aV]], pos: side[aV]};
